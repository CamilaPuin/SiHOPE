/**
 * Isotipo de SiHope: nodo central conectado a los tres actores del modelo de
 * monitorías (monitor, conocimiento, estudiante). Portado del fragmento
 * `isotipo` de layout.html.
 *
 * @param {boolean} negativo  usa la variante en negativo (para fondos oscuros).
 */
export default function Isotipo({ negativo = false, className = "" }) {
    const clases = ["iso", negativo ? "iso--neg" : "", className]
        .filter(Boolean)
        .join(" ");
...
/** Logotipo "SiHope" (wordmark). `negativo` para fondos oscuros. */
export function Wordmark({ negativo = false }) {
    return (
        <span className={`wordmark ${negativo ? "wordmark--neg" : ""}`}>
